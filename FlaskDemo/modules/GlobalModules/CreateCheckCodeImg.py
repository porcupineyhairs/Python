...
class CreateCheckCode:
	def __init__(self, width=150, height=35, code_count=4, font_type='static/font/font.ttf', font_size=32,
...
def getCheckCodeImgPath(ip=''):
	...
	filePath = 'static/others/checkCode/{}.png'.format(str(timeStr) + '-' + ip)

	f = open(file=filePath, mode='wb')
	f.write(data)
	f.close()
	del img
	return '/' + filePath, valid_str.upper()
