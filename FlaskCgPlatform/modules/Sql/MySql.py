...
class MySqlHelper:
	def __doc__(self):
		return 'Class Name: MySqlHelper \n' \
		       'This the class to use MsSql for Select, Update, Delete, Insert and so on. \n' \
		       'Author：Harvey \n' \
		       'CreateDate：2020.04.06 '

	def __init__(self, host=None, port=3306, user=None, passwd=None, database=None, charset='utf8'):
		self.__host = host
		self.__port = port
		self.__user = user
	...
	def __createObj(self):
		if (self.__host is None or self.__host == '' or self.__user is None or self.__user == '' or
			...
			raise MySqlHelperException('Input Parameter is Error.')

		self.__conn = pymysql.connect(host=self.__host, user=self.__user, password=self.__passwd, database=self.__db,
	...
	def __work(self):
		if self.__sqlStr is None:
			raise MySqlHelperException('Sql Str is None.')
		# 根据SQL第一个关键字获取模式
		...
		if self.__sqlMode in ('SELECT', 'EXEC', 'IF'):
			...
			raise MySqlHelperException('Can Not Get Sql Mode, Please Check Sql Sentence. Sql: ' + self.__sqlStr)
...
def test():
	# 连接database
	conn = pymysql.connect(host='127.0.0.1', user='root', password='Tiamohui', database='WebApp', charset='utf8')
