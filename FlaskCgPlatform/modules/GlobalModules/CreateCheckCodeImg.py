...
class CreateCheckCode:
	def __init__(self, width=120, height=30, code_count=4, font_type=sys.path[0] + '/static/font/font.ttf', font_size=28,
	...
	def getValidCodeImg(self):
		...
		font = ImageFont.truetype(self.font_type, size=self.font_size)
...
def getCheckCodeImgPath(ip=''):
	timeStr = datetime.now().strftime('%Y%m%d-%H%M%S')
	img = CreateCheckCode()
	data, valid_str = img.getValidCodeImg()
	idx = random.randint(1, 100)
	filePath = sys.path[0] + '/static/others/checkCode/{0}.png'.format(str(timeStr) + '-' + ip + '-' + str(idx))

	f = open(file=filePath, mode='wb')
	f.write(data)
	f.close()
	del img
	return '/static/others/checkCode/{0}.png'.format(str(timeStr) + '-' + ip + '-' + str(idx)), valid_str.upper()
