...
if len(sys.argv) < 2:
	print('用于下载QQ音乐的文件。有了这个再也不担心下载音乐要开通各种钻了')
	print(' NO action specified.')
	print(u' 请输入参数.如下')
	print(u' https://y.qq.com/n/yqq/playlist/4108153914.html')
	sys.exit()
...
def save_mp3(music_name, res, word):
	# 保存下载文件
	filePathTmp = '/Users/harvey/Downloads/'
	# filePath = os.path.join(os.getcwd(), word)
	filePath = os.path.join(filePathTmp, word)
	if not os.path.exists(filePath):
		print('保存路径不存在')
		os.makedirs(filePath)
		print('保存创建成功')
		with open(filePathTmp + word + "/" + music_name, "wb") as file:
			file.write(res)
	else:
		with open(filePathTmp + word + "/" + music_name, "wb") as file:
			file.write(res)
...
for n in range(0, len(mids)):
	resp2 = requests.get(
		...
		songmids[n] + '&filename=M500' + mids[n] + filetype + '&guid=6612300644')
